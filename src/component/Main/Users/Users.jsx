...
const Users = () => {
  ...
  const { data } = useGetAllUsersQuery({ from: fromDate, to: toDate, searchData });
  ...
  const [dataSource, setDataSource] = useState(fullUserData); // Initialize with demo data
  ...
  useEffect(() => {
    if (searchText.trim() === "") {
      setDataSource(fullUserData); // Reset to all users
    } else {
      setDataSource(
        fullUserData?.filter(
          (user) =>
  ...
  useEffect(() => {
    if (!selectedDate) {
      setDataSource(fullUserData); // Reset to all users if no date is selected
    } else {
